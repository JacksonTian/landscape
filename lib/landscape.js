...
(function (global) {
  ...
  /**
   * 给前Land视图绑定一个Scape对象。
   * 通过添加ld-bind属性到DOM元素中，使得DOM上的改动能够设置回Scape对象上。
   */
  Land.prototype.bind = function (scape) {
    this.el.delegate('[ld-bind]', 'change', function (event) {
      var current = $(event.currentTarget);
      var key = current.attr('ld-bind');
      scape.set(key, current.val());
    });
    return this;
  };
   ...
   * 返回Scape对象中的数据
   */
  Scape.prototype.toData = function () {
    return this.data;
  };
